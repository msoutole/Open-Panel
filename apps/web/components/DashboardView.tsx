...
import { Project, MetricPoint, CPU_DATA, NETWORK_DATA } from '../types'; // Assuming these types and data are defined elsewhere
import CreateProjectModal from './CreateProjectModal'; // Assuming this component exists
...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, onClick }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:border-primary/30 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group cursor-pointer relative overflow-hidden h-full flex flex-col" onClick={onClick}>
      <div className="flex items-start justify-between mb-4 relative z-10">
        <div className="flex items-center gap-4">
          <div className={`w - 14 h - 14 rounded - 2xl flex items - center justify - center transition - colors shadow - sm border border - slate - 100 ${
      isHealthy? 'bg-gradient-to-br from-blue-50 to-white text-blue-600 group-hover:from-blue-600 group-hover:to-blue-500 group-hover:text-white': 'bg-gradient-to-br from-orange-50 to-white text-orange-600 group-hover:from-orange-600 group-hover:to-orange-500 group-hover:text-white'
    }`}>
            <Layers size={26} strokeWidth={1.5} />
          </div>
          <div>
...
const ProjectListItem: React.FC<ProjectCardProps> = ({ project, onClick }) => {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 flex-1 min-w-0">
        <div className={`w - 12 h - 12 rounded - xl flex items - center justify - center transition - colors shadow - sm border border - slate - 100 shrink - 0 ${
      isHealthy? 'bg-blue-50 text-blue-600 group-hover:bg-blue-600 group-hover:text-white': 'bg-orange-50 text-orange-600 group-hover:bg-orange-600 group-hover:text-white'
    }`}>
          <Layers size={20} strokeWidth={2} />
        </div>
        <div className="min-w-0">
