...
export const EditProjectModal: React.FC<EditProjectModalProps> = ({ isOpen, onClose, onUpdated, project }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        slug: '',
        description: '',
        type: 'WEB' as 'WEB' | 'API' | 'DATABASE' | 'WORKER'
    });
    ...
    useEffect(() => {
        if (project && isOpen) {
            setFormData({
                ...
                type: (project.type as 'WEB' | 'API' | 'DATABASE' | 'WORKER') || 'WEB'
            });
        }
    }, [project, isOpen]);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="bg-card rounded-xl shadow-lg w-full max-w-lg overflow-hidden">
                <div className="px-6 py-4 border-b border-border flex items-center justify-between bg-background">
                    <h3 className="font-bold text-lg text-textPrimary">{LL.common.edit()} {LL.projects.title()}</h3>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-textPrimary mb-1.5">{LL.common.name()}</label>
                            <input
                                type="text"
                                required
                                value={formData.name}
